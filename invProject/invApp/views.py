...
def dashboard(request):
	...
	total_quantity = all_products.aggregate(total=Sum("quantity"))["total"] or 0
	total_value = sum((product.price * product.quantity for product in all_products), 0)
	...
	context = {
		...
		"product_form": ProductForm(owner=request.user),
...
def product_create(request):
	if request.method == "POST":
		...
		if form.is_valid():
			...
			messages.error(request, "Could not create product. Check form fields.")
...
def product_edit(request, product_id):
	...
	if request.method == "POST":
		...
		if form.is_valid():
			...
			messages.error(request, "Could not update product. Check form fields.")
	return redirect("dashboard")
...
def export_products_csv(request):
	...
	writer.writerow(["Name", "Category", "SKU", "Price", "Quantity", "Supplier", "Low Stock"])

	for product in Product.objects.filter(owner=request.user).select_related("category"):
		writer.writerow(
			[
				product.name,
...
def export_products_pdf(request):
	...
	for product in Product.objects.filter(owner=request.user).order_by("name"):
		line = f"{product.name} | {product.sku} | Qty: {product.quantity} | ${product.price:.2f}"
